...
<link rel="stylesheet" href="/twwechat/Function/2016ggstar/Public/Css/progress_bar.css" />
</head>
<body>
	<!-- 第一页 -->
  <!-- 第一页 -->
...
<!-- /header -->
    ...
    <div data-role="content">
      <?php $count=0;$themeid='d'; ?>
      <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><ul data-role="listview" data-inset="true" data-theme="c">
      <li data-role="list-divider" data-theme="<?php echo ($themeid); ?>"><?php echo ($groupname[$count]["group"]); ?></li>
      <?php if(is_array($vo)): $i = 0; $__LIST__ = $vo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 3 );++$i; if(($mod) == "1"): ?><li>
        <a><img src="/twwechat/Function/2016ggstar/Public/images/logo/<?php echo ($data["voter_id"]); ?>.jpg"><?php echo ($data["name"]); ?><div class="progress">
          <div class="progress-bar progress-bar-success progress-bar-striped" role="progressbar" style="width: <?php echo ($data['count']/50+10); ?>%">
            <?php echo ($data["count"]); ?>
          </div>
        </div></a>
      </li><?php endif; ?>
      <?php if(($mod) == "2"): ?><li>
        <a><img src="/twwechat/Function/2016ggstar/Public/images/logo/<?php echo ($data["voter_id"]); ?>.jpg"><?php echo ($data["name"]); ?><div class="progress">
          <div class="progress-bar progress-bar-warning progress-bar-striped" role="progressbar" style="width: <?php echo ($data['count']/50+10); ?>%">
            <?php echo ($data["count"]); ?>
          </div>
        </div></a>
      </li><?php endif; ?>
      <?php if(($mod) == "0"): ?><li>
          <a><img src="/twwechat/Function/2016ggstar/Public/images/logo/<?php echo ($data["voter_id"]); ?>.jpg"><?php echo ($data["name"]); ?><div class="progress">
            <div class="progress-bar progress-bar-danger progress-bar-striped" role="progressbar" style="width: <?php echo ($data['count']/50+10); ?>%">
              <?php echo ($data["count"]); ?>
            </div>
          </div></a>
